...
namespace LegendaryLibrary
{
    public partial class frmPhoenixCheck : Form
    {
        ...
        private void btnRunCheck_Click(object sender, EventArgs e)
        {
            try
            {
                Legendary.UpdateStatus("Phoenix File by CU Account Loading");
                ...
                Legendary.UpdateStatus("Phoenix File by Subscription Index Loading");
                ...
                Legendary.UpdateStatus("Phoenix File by Broker Dealer Loading");
                ...
                Legendary.UpdateStatus("Phoenix File by Rep Name Loading");
                ...
                Legendary.UpdateStatus("\r\n\r\n");
                Legendary.UpdateStatus("--------------------------------------------------------------------------");
                Legendary.UpdateStatus("                       DOING COMPARISON CHECK NOW !!                      ");
                Legendary.UpdateStatus("--------------------------------------------------------------------------");
                Legendary.UpdateStatus("   ");
                                         ...
                                         string matchCriteriaName, string matchCriteriaValue)
        {
            ...
            try
            {
                ...
                Legendary.UpdateStatus($"{otherListName} {otherField} \"{otherValue}\" vs \"{phoenixValue}\" " +
                                       ...
                                       string matchCriteriaName, string matchCriteriaValue)
        {
            ...
            try
            {
                ...
                if (!double.TryParse(phoenixValue, out phoenixDouble))
                    Legendary.UpdateStatus($"Parsing of Phoenix {phoenixField} for {matchCriteriaName} '{matchCriteriaValue}' failed.  Skipping comparison.");
                else if (!double.TryParse(otherValue, out otherDouble))
                    Legendary.UpdateStatus($"Parsing of {otherListName} {otherField} for {matchCriteriaName} '{matchCriteriaValue}' failed.");
                else
                {
                    ...
                    Legendary.UpdateStatus($"{otherListName} {otherField} {otherDate.ToShortDateString()} does not equal " +
                ...
                else
                {
                    ...
                    else
                    {
                        if (phoenixDouble != (otherDouble * otherMultiplier))
                            Legendary.UpdateStatus($"{otherListName} {otherField} {otherDouble.ToString("0.00")} does not equal " +
        ...
        private bool ComparePhoenixToISS()
        {
            ...
            foreach (var phoenixPair in PhoenixByCU)
            {
                ...
                {
                    Legendary.UpdateStatus($"    Cannot Find CAP Account {phoenixPair.Key} In ISS Data, continuing to next row");
                    continue;
                }
                ...
                if (string.Compare(phoenixValue, issValue, true) != 0)
                    Legendary.UpdateStatus($"Social Security --> For CAP Account: {phoenixPair.Key} The ISS[Account] {issValue} " + 
            ...
            Legendary.UpdateStatus("Phoenix to ISS Comparison is Complete (see above for issues)");
        ...
        private bool ComparePhoenixToCommission()
        {
            ...
            foreach (var phoenixPair in PhoenixBySubIndex)
            {
                ...
                if (!CommissionBDR.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find Subscription Index {phoenixPair.Key} In Commission Broker Dealer Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
                ...
                if (!CommissionMarketing.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find Subscription Index {phoenixPair.Key} In Commission Marketing Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
            ...
            Legendary.UpdateStatus("Phoenix to Commission Comparison is Complete (see above for issues)");
        ...
        private bool ComparePhoenixToMasterDistribution()
        {
            ...
            foreach (var phoenixPair in PhoenixByCU)
            {
                ...
                if (!MasterDistribution.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find CU Account {phoenixPair.Key} In Master Distribution Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
                    ...
                    if (string.Compare(phoenixCityStateZip.Trim(), masterDistributionCityStateZip.Trim(), true) != 0)
                        Legendary.UpdateStatus($"Master Distribution CITY STATE ZIP CODE '{masterDistributionCityStateZip}' vs '{phoenixCityStateZip}' " +
        ...
        private bool ComparePhoenixToCapitalRaiseDashboard()
        {
            ...
            foreach (var phoenixPair in PhoenixBySubIndex)
            {
                ...
                if (!CRDashboardSubscriptions.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find Sub Index \"{phoenixPair.Key}\" In Capital Raise Dashboard Subscriptions Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
            ...
            foreach (var phoenixPair in PhoenixByBD)
            {
                ...
                if (!CRDashboardBDs.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find Orig Broker Name \"{phoenixPair.Key}\" In Capital Raise Dashboard Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
                    ...
                    if (string.Compare(phoenixAddress.Trim(), capitalRaiseBDAddress.Trim(), true) != 0)
                        Legendary.UpdateStatus($"Capital Raise Dashboard BDs ADDRESS '{capitalRaiseBDAddress}' vs '{phoenixAddress}' " +
                    ...
                    if (string.Compare(phoenixBDCityStateZip.Trim(), capitalRaiseBDCityStateZip.Trim(), true) != 0)
                        Legendary.UpdateStatus($"Capital Raise Dashboard BDs CITY STATE ZIP CODE '{capitalRaiseBDCityStateZip}' vs '{phoenixBDCityStateZip}' " +
            ...
            foreach (var phoenixPair in PhoenixByRep)
            {
                ...
                if (!CRDashboardReps.ContainsKey(phoenixPair.Key))
                    Legendary.UpdateStatus($"    Cannot Find Producing Broker Rep \"{phoenixPair.Key}\" In Capital Raise Dashboard Data, continuing...");
                else
                {
                    phoenixList = phoenixPair.Value;
                    ...
                    if (string.Compare(phoenixAddress.Trim(), capitalRaiseRepAddress.Trim(), true) != 0)
                        Legendary.UpdateStatus($"Capital Raise Dashboard Reps ADDRESS '{capitalRaiseRepAddress}' vs '{phoenixAddress}' " +
                    ...
                    if (string.Compare(phoenixRepCityStateZip.Trim(), capitalRaiseRepCityStateZip.Trim(), true) != 0)
                        Legendary.UpdateStatus($"Capital Raise Dashboard Reps CITY STATE ZIP CODE '{capitalRaiseRepCityStateZip}' vs '{phoenixRepCityStateZip}' " +
            ...
            Legendary.UpdateStatus("Phoenix to Capital Raise Dashboard Comparison is Complete (see above for issues)");
