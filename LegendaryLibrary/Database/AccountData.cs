            ...
            using (SqlConnection connection = new SqlConnection(Legendary.GetSqlConnectionString()))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        int ledger_Account_Id = (reader[1] is DBNull) ? 0 : reader.GetInt32(1);
                        int entity_Id = (reader[2] is DBNull) ? 0 : reader.GetInt32(2);
                        DateTime value_Date = (reader[3] is DBNull) ? DateTime.MinValue : reader.GetDateTime(3);
                        decimal value = (reader[4] is DBNull) ? (decimal)0.00 : reader.GetDecimal(4);
                        int year = (reader[5] is DBNull) ? 0 : reader.GetInt32(5);
                        int month = (reader[6] is DBNull) ? 0 : reader.GetInt32(6);
                        int day = (reader[7] is DBNull) ? 0 : reader.GetInt32(7);
