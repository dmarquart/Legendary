...
namespace LegendaryLibrary
{
    public class LedgerAccount
    {
        ...
        static public LedgerAccount GetLedgerAccountByAccountNumberAndEntityTypeAndName(string accountNumber, string entityType, string accountName, List<LedgerAccount> ledgerAccounts)
        {
            LedgerAccount ledgerAccount = null;
            foreach (var account in ledgerAccounts)
            {
                if ( (string.Compare(account.Number, accountNumber, true) == 0) && 
                      account.Entity_Type.Contains(entityType.ToUpper()) &&
                     (string.Compare(account.Name, accountName, true) == 0) )
                {
                    ledgerAccount = account;
                    break;
                }
